...
if($_GET["action"] == "edit"){
    ...
    if(!empty(trim($_POST["doctor"])) && !empty(trim($_POST["visit_date"]))){
        $pdo_statement = $DBH->prepare("UPDATE appointments SET doctor=:doctor, visit_date=:visit_date, symptoms=:symptoms WHERE id=:id");
        ...
        $pdo_statement = $DBH->prepare("SELECT * FROM appointments WHERE id=?");
